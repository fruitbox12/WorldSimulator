...
void Tools::setFullScreen() {
	...
	COORD coordinates = { BUFFER_WIDTH, BUFFER_HEIGHT };
...
Tree<std::string, std::string>* Tools::XMLParser(std::string filePath, std::string fileName, std::vector<std::string> &messages) {
	// Create a temporary empty root tree. It will be deleted once we are ready to create the full root tree.
	// If the XML file is not found we will return this non-null empty root tree.
	Tree<std::string, std::string>* rootTree = new Tree<std::string, std::string>();
	...
	if (fileExists(filePath, fileName)) {
		...
		for (auto iterator = text.begin(); iterator != text.end(); ++iterator) {
			...
			int openCharPosition = (*iterator).find_first_of('<');
			int trimmedLength = (*iterator).length() - openCharPosition;
			...
			// Check for comment tags
				...
				int closeCharPosition = (*iterator).find("-->", 0);
		...
		for (auto iterator = data.begin(); iterator != data.end(); ++iterator) {
			std::string tagName = "";
			std::string tagText = "";
			Tree<std::string, std::string>* subtree = nullptr;
			...
			if (validateTag(data, iterator, closeTagIterator, tagName, errors, warnings)) {
				...
				if (isATree(data, iterator, closeTagIterator, tagName, errors, warnings)) {
					...
					if (!rootTagFound) {
						// Delete the temporary empty root tree created above
						delete rootTree;

						// Translate the tag name to lower case
						rootTree = new Tree<std::string, std::string>(currentLevel, Tools::toLower(tagName), true);
						rootTagFound = true;
						...
						// TEST ONLY, WILL BE REMOVED LATER. WE SHOULD NEVER GET HERE
						...
						// Found a subtree of the root tree
						// Translate the tag name to lower case
						subtree = new Tree<std::string, std::string>(currentLevel, Tools::toLower(tagName), true);

						addNodes(data, iterator, closeTagIterator, subtree, currentLevel, errors, warnings);
					}
				}
				else {
					// Check if the root tree has already been found
					if (!rootTagFound) {
						errors.emplace_back("Error: missing root tag. First tag must be a tree");
						//*** TEST THIS! ***
		...
		std::cout << "Tools::XMLParser(): unable to find file " << filePath + fileName << std::endl;
	}
...
bool Tools::validateTag(std::vector<std::string> &data, std::vector<std::string>::iterator iterator, std::vector<std::string>::iterator &closeTagIterator, std::string &tagName, std::vector<std::string> &errors, std::vector<std::string> &warnings) {
	bool valid = false;
		...
		if ((*iterator).substr(0, 1) == "<") {
			...
			if (closePosition != std::string::npos) {
				...
				if (closePosition >= 2) {
					...
					std::string name = (*iterator).substr(1, closePosition - 1);
					...
					if (closeTagIterator != data.end()) {
						...
						// Missing close tag
						errors.push_back("Error, missing close tag for " + (*iterator));
					}
				}
				else {
					// Missing tag name
					errors.push_back("Error, missing tag name on open tag <" + (*iterator) + ">");
				}
			}
			else {
				// Missing close > on open tag
				errors.push_back("Error: missing close > char on open tag " + (*iterator));
			}
		}
		else {
			// Missing open < on open tag
			errors.push_back("Error: missing open < char on open tag " + (*iterator));
		}
	}
	else {
		errors.push_back("Error, missing open tag for " + (*iterator));
	}
...
void Tools::addNodes(std::vector<std::string> &data, std::vector<std::string>::iterator& iterator, std::vector<std::string>::iterator &closeTagIterator, Tree<std::string, std::string>* tree, int currentLevel, std::vector<std::string> &errors, std::vector<std::string> &warnings) {	
	...
	if (iterator + 1 < data.end()) {
		...
		for (++iterator; iterator < closeTagIterator; ++iterator) {
			...
			if (validateTag(data, iterator, closeIterator, tagName, errors, warnings)) {
				...
				if (isATree(data, iterator, closeIterator, tagName, errors, warnings)) {
					...
					// Translate the tag name to lower case
					subtree = new Tree<std::string, std::string>(tree->getLevel() + 1, Tools::toLower(tagName), true);
					...
					if (tagText.length() == 0) {
						warnings.push_back("Warning: tag <" + tagName + "> has no text");
					}
		...
		errors.push_back("Error: empty tree: " + (*iterator));
	}
}

bool Tools::isATree(std::vector<std::string> &data, std::vector<std::string>::iterator iterator, std::vector<std::string>::iterator &closeTagIterator, std::string &tagName, std::vector<std::string> &errors, std::vector<std::string> &warnings) {
	bool isATree = true;
...
void Tools::addNodesRecursive(std::vector<std::string> &data, std::vector<std::string>::iterator& iterator, std::vector<std::string>::iterator &closeTagIterator, Tree<std::string, std::string>* tree, int currentLevel, std::vector<std::string> &errors, std::vector<std::string> &warnings) {
	// TEST PAST END OF TEXT
	if (iterator + 1 < data.end()) {
		// Process node tags
